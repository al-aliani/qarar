...
describe('ExcelExporter — ورقة التدفقات النقدية تجمع فعلاً إلى صافي التدفق المطبوع', () => {
    ...
    it('سداد أصل القرض والاستثمار ودخول القرض لها صفوف، والعمود يتوازن في كل سنة بما فيها سنة الصفر', async () => {
        ...
        const componentRows = rows.filter((r) => r[0] !== 'صافي التدفق النقدي'
            ...
            // إضافة تدقيق 2026-09-16: صفّا ضريبة القيمة المضافة تسوية منفصلة بعد صافي
            // التدفق، لا مكوّنين إضافيين يُجمَعان إليه — استبعادهما هنا يبقي هذا الاختبار
            // مختصاً بهوية «المكوّنات الأصلية تجمع لصافي التدفق» فقط.
            && r[0] !== 'صافي ضريبة القيمة المضافة المستحقة'
            && r[0] !== 'التدفق النقدي بعد ضريبة القيمة المضافة');
