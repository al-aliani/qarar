...
describe('SensitivityAnalysis — جدول مستويات التشغيل: تناسق اللون مع فشل الحساب (خلل مُصلَح)', () => {
    ...
    it('حارس انحدار: كل سيناريو (-10%/-20%/-30%) يُستدعى مرة واحدة فقط لكل render (لا نداء مضاعف كما كان)', () => {
        ...
        // -30% تظهر فقط في جدول مستويات التشغيل (نداء واحد). -20% تظهر في المصفوفة
        // + جدول المستويات (نداءان). -10% تظهر في بطاقة الحساسية + المصفوفة + جدول
        // المستويات (3 نداءات). المهم: لا تضاعف داخل *نفس* الجدول (لا نداءان لكل خلية
        // كما كان قبل الإصلاح في جدول مستويات التشغيل تحديداً).
        expect(countFor(-0.30)).toBe(1);
        expect(countFor(-0.20)).toBe(2);
        expect(countFor(-0.10)).toBe(3);
    });
