...
export class SubscriptionCheckoutView {
    ...
    _renderCheckout() {
        ...
        const tiersHtml = paidPackages.map(p => `
            <button type="button" class="checkout-tier ${p.id === pkg.id ? 'is-active' : ''}" data-tier="${p.id}">
                <span class="checkout-tier__name">${escapeHtml(p.name)}</span>
    ...
    _addonRowHtml(a, pkg) {
        ...
        return `<label class="flex justify-between items-center py-2" data-addon-row="${a.id}"><span><input type="checkbox" data-addon="${a.id}" data-price="${a.price}"${included ? ' checked disabled' : ''}> ${escapeHtml(a.name)}<span class="text-xs text-muted" data-addon-note style="${included ? '' : 'display:none'}"> (مشمولة في باقتك)</span></span><span>${a.price} ريال</span></label>`;
    }
