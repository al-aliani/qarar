...
/**
 * قرار لجنة 2026-08-27 (خبير ثقة عملاء + مهندس + مدير منتج، 3 آراء مستقلة):
 * فتح دراسة قديمة بعد تعديل معادلة هنا (breakeven، زكاة، إهلاك...) يُعيد حسابها
 * صامتاً بالمعادلة الجديدة بلا أي أثر — عميل صدَّر PDF لبنك برقم NPV معيّن قد
 * يرى رقماً مختلفاً لاحقاً بلا تفسير. **ارفع هذا الرقم يدوياً مع أي تعديل يغيّر
 * ناتج calculateStudy لدراسة قائمة** (لا تغييرات الواجهة/الأداء البحتة). يُخزَّن
 * في PersistenceService._meta.engineVersion وقت الحفظ، ويُقارَن عند العرض في
 * ProjectOverviewView._renderEngineVersionNotice. حل جزئي متعمَّد (بصمة+تنبيه لا
 * تجميد فعلي للنتائج) — التصدير نفسه يبقى يعيد الحساب حياً؛ التجميد الكامل
 * (سجل تاريخي ثابت وقت الدفع/التصدير) بوابة إلزامية قبل أول دفعة عميل حقيقي
 * تُصدَّر منها دراسة، لا بند "لاحقاً" عام — انظر output/2026-08-27/DECISIONS.md.
 */
// لا علاقة لهذا بـschema.js:version ("4.0.0") — ذاك يصف شكل/بنية كائن الدراسة
// نفسه (schema)، هذا يصف منطق الحساب (calculateStudy) — رقمان مستقلان عمداً.
