...
export function buildRevenueModel(ctx) {
    ...
    // مصادر الإيرادات: التشغيلية تُستبدل بالخدمات إن وُجدت — هذا الشرط أُزيل سهواً في
    // جلسة سابقة الليلة (تدقيق 2026-07-17) بدعوى "معالجة تراكب صامت"، لكن إزالته فعلياً
    // ضاعفت الإيراد (كلا المصدرين يُجمعان معاً لنفس الدخل الحقيقي) — تحقّق حي على دراسة
    // فعلية أظهر NPV مُضخَّماً زوراً من سالب صحيح إلى +1.6 مليون بمجرد هذا التغيير.
    // أُعيد الشرط الأصلي؛ المعالجة الصحيحة لتراكب services/revenue.streams تبقى مهمة
    // متابعة منفصلة (توضيح للمستخدم أيّهما مصدر الحقيقة، لا جمع الاثنين حسابياً).
