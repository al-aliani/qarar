 ...
 * مع نفسها.
 *
 * تصحيح (تدقيق شامل 2026-09-16): الادّعاء السابق هنا بأن «الاختلال غير قابل للتوليد من
 * مُدخَلات المستخدم اليوم» كان خاطئاً فعلاً — قرض بمدة أقصر من فترة سماحه (مثال حقيقي ضمن
 * حدود الواجهة: termYears=2, gracePeriodMonths=24) كان يُسقط رصيد القرض من الميزانية
 * بمجرد تجاوز termYears، مُصلَح الآن في lib/calc/balanceSheet.js (انظر
 * lib/calc/__tests__/balanceSheet.loanBalanceCarryForward.test.js للاختبار الحقيقي غير
 * المحقون). هذا الملف يبقى مفيداً رغم ذلك لغرض مختلف: اختبار **عقد البوابة نفسها** (ماذا
 * تفعل حين تصلها سنة غير متوازنة أياً كان سبب الاختلال) بمعزل عن أي علة حسابية بعينها —
 * لا ضمان أن كل مسارات الاختلال المحتملة مُغلقة، فالحقن على حدود الوحدة (generateBalanceSheets)
 * يبقى الطريقة الصحيحة لاختبار عقد البوابة تحديداً.
 *
