...
describe('أوضاع تفصيل الدراسة — انكماش التصنيفات في صفحة الفئات (منطق app.js)', () => {
    ...
    it('مصغّر ينكمش لتصنيفات قليلة غير فارغة (لا صفحات «لا توجد أقسام» في التنقل)', () => {
        // تدقيق 2026-07-17: SIDEBAR_SECTIONS أُعيد ترتيبه (أهمية + تتابعية) — الفهارس هنا
        // الآن: 0=تعريف 1=سوق 2=تشغيل 3=مالية 4=استراتيجي 5=تنفيذ 6=أدلة 7=نتائج.
        const cats = visibleCategoryIndexes('mini');
        // مصغّر يمسّ: التعريف(0)، السوق/الإيراد(1)، التشغيل(2)، المالية(3)، النتائج(7)
        expect(cats).toEqual([0, 1, 2, 3, 7]);
        // التصنيفات الفارغة (الاستراتيجي/خطة التنفيذ/الأدلة) تُستبعد من التنقل
        expect(cats).not.toContain(4);
