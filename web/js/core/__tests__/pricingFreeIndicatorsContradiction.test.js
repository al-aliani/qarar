...
describe('تناقض: عرض المؤشرات مجاني، لكن نص الترقية كان يدّعي أنه محجوب', () => {
    ...
    it('[إثبات الحارس] القيمة الحقيقية المستوردة من pricing.js الآن ليست الصياغة القديمة (لو رجعت، الفحص أعلاه يفشل فعلياً)', () => {
        const oldUnlock = 'المؤشرات والتصدير الاحترافي ومراجعة الخبير'; // الصياغة القديمة الفعلية قبل هذا الإصلاح
        // نقارن القيمة الحقيقية المستوردة أعلاه من pricing.js (لا نسخة مصطنعة منفصلة)
        // بالنص القديم مباشرة — لو رجع pricing.js لصياغته القديمة تفشل هذه المقارنة.
        expect(PRICING_COMPARISON.free.unlock).not.toBe(oldUnlock);
    });
});
