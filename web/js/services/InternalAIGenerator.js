 ...
 * @returns {string}
 */
export function generateFieldSuggestion(fieldName, currentValue, state, tone = 'professional') {
    const p = state?.projectInfo || {};
    const name = or(p.name, 'المشروع');
    const concept = shortActivity(p, 'النشاط');
    const city = or(p.city, 'المنطقة');
    ...
    if (hasExistingText) {
        // Expand Mode
        baseText = `بناءً على الفكرة الأولية "${currentValue.trim()}"، نهدف في مشروع «${name}» لتقديم تجربة استثنائية في قطاع ${concept} في ${city}، مع التركيز على تلبية احتياجات العملاء المستهدفين وتحقيق ميزة تنافسية مستدامة.`;
    } else {
        const templates = {
        ...
        baseText = templates[key] || templates.description;
    }

    // Apply Tone Formatting
    if (tone === 'creative') {
        baseText = `🌟 مرحباً بك في المستقبل مع مشروعنا! ${baseText.replace('المشروع', '').replace('يهدف إلى', 'نسعى بشغف لـ')}`;
    } else if (tone === 'bullets') {
        baseText = `📌 أبرز النقاط:\n- ${baseText.split('،').join('\n- ')}`;
    }
