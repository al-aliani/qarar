...
describe('privacy.html / data-retention.html — النص يطابق الواقع الفعلي للكود', () => {
    ...
    it('لا يعد بحذف فوري آلي عند "إغلاق الحساب أو طلب حذفه" — التسجيل فوري، والحذف الفعلي بعد فترة سماح 7 أيام قابلة للتراجع', () => {
        // تحديث 2026-08-29 (تفويض مالك صريح): كان الوعد السابق "لا نضمن مدة زمنية
        // محددة" صادقاً حين كانت المعالجة يدوية بلا كود فعلي (DECISIONS.md §4). صار
        // للحذف الآن تنفيذ فعلي حقيقي (process-account-deletions، مجدولة يومياً) —
        // إبقاء صياغة "بلا مدة مضمونة" بعد بناء الآلية الفعلية يصبح هو نفسه ادّعاءً
        // غير صادق (تناقض SLA حقيقي قائم فعلاً)، فالنص يجب أن يعكس الرقم الحقيقي.
        ...
        expect(retentionHtml).not.toMatch(/لا نضمن حالياً مدة زمنية محددة/);
        expect(retentionHtml).not.toMatch(/يدوياً من فريقنا/);
        expect(retentionHtml).toMatch(/تقديم طلب حذف الحساب يُسجَّل في نظامنا فوراً/);
        expect(retentionHtml).toMatch(/فترة سماح مدتها 7 أيام/);
        expect(retentionHtml).toMatch(/خلال 7 إلى 8 أيام من تقديم الطلب/);
    });
    ...
    it('نفس التصحيح مطبَّق في privacy.html §5 "حقوقك" أيضاً — كانت تحمل نفس الادّعاء الخاطئ بصياغة مستقلة فاتت المراجعة الأولى', () => {
        ...
        expect(privacyHtml).not.toMatch(/بلا مدة زمنية مضمونة حالياً/);
        expect(privacyHtml).toMatch(/التواصل معنا لطلب حذف حسابك/);
        expect(privacyHtml).toMatch(/يُسجَّل طلبك فوراً/);
        expect(privacyHtml).toMatch(/فترة سماح مدتها 7 أيام/);
    });
