...
تبعات: 11 ملف اختبار e2e احتاجت تحديثاً (مساعد مشترك `e2e/helpers/auth.js`) — 35/43 أخضر (chromium)،
الباقي مؤكَّد غير مرتبط بهذا التغيير (راجع رسالة commit `fix(e2e): ...` للتفصيل). `visual.spec.js`
يحتاج `--update-snapshots` بعد مراجعة بصرية إن أردت إصلاحه (لقطاته المرجعية تعكس المحتوى القديم بلا تسجيل دخول).
...
- الكومِتات الأربعة (`ba10c504`، `7d83e144`، `17d6a2c5`، `bb551540`) **لم تُدفَع لـ`origin/main` بعد** — بانتظار تأكيد.
