...
- **دوال المراجعين الثلاث** (`reviewer-queue/claim/submit`) — **منشورة وحيّة** (كانت 404 لغياب خطوة نشر بالـCI فقط، لا مشكلة كود/DB). **ناقص تشغيلياً:** لا يوجد صفّ فعّال بجدول `reviewers` بعد — الباقة تُباع بلا مراجع فعلي حتى تُضاف واحد يدوياً.
- **Sentry** (`VITE_SENTRY_DSN`) — مُفعَّل بالإنتاج، تحقَّق من الملف المنشور فعلياً.

**⚠️ المتبقي الوحيد الجوهري — اختبار الدفع الحيّ e2e لم يُنفَّذ فعلياً بعد:**
كُتب اختبار Playwright حقيقي ([`e2e/payment_bank_transfer.spec.js`](e2e/payment_bank_transfer.spec.js)) يغطي المسار كاملاً (تسجيل دخول → دراسة → تحويل بنكي → ظهوره بلوحة الأدمن)، لكنه **لم يُشغَّل قط** — يحتاج حساب عميل مؤكَّد البريد (`E2E_CUSTOMER_EMAIL`/`PASSWORD`) لا يوجد حالياً. هذا يبقى المسار الوحيد غير المُتحقَّق منه حيّاً في كل منظومة الدفع.
...
1. **شغّل اختبار الدفع e2e المكتوب فعلاً** ([`e2e/payment_bank_transfer.spec.js`](e2e/payment_bank_transfer.spec.js)) — أنشئ مستخدماً تجريبياً مؤكَّد البريد بلوحة Supabase (Authentication → Add user، فعّل Auto Confirm)، صدّر `E2E_CUSTOMER_EMAIL`/`E2E_CUSTOMER_PASSWORD`، وشغّله. هذا آخر مسار غير مُتحقَّق منه حيّاً بمنظومة الدفع كاملةً — راجع §4.
2. **أضف مراجعاً فعّالاً واحداً على الأقل** لجدول `reviewers` (`active=true`) — دوال المراجعين منشورة وحيّة لكن باقة «مراجَع بخبير 1,999» تُباع بلا أحد يقدر يستخدم المسار.
3. Word/PPTX لا يزالان يختصران بعض المبالغ الكبيرة لـ"X.X مليون ريال" في مسارات متبقية (أثر منخفض) — راجع تاريخ git لـ`web/export/wordExporter.js`/`pptxExporter.js` إن أردت إغلاقها بالكامل.
