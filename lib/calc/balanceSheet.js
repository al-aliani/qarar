...
export function computeBalanceSheet(data, year) {
    const {
        ...
        cashCycle = null,
        // آخر سنة في أفق الدراسة (يُمرَّرها generateBalanceSheets) — تُفعِّل تصفية رأس
        // المال العامل أدناه لتطابق افتراض «التصفية» الذي تفترضه قائمة التدفقات بالفعل.
        isLastYear = false
    } = data;
    ...
    // تدقيق 2026-08-25: كان `capex.subtotal || capex.total` — و«0» قيمة falsy، فأي دراسة
    // خفيفة الأصول (خدمات/استشارات بلا معدات أو مبانٍ) وsubtotal=0 مشروع كانت تسقط إلى
    // capex.total = totalCapex + رأس المال العامل + بضاعة أول المدة (engine.js:1137)، فيظهر
    // رأس المال العامل أصلاً ثابتاً وهمياً — وهو محتسَب أصلاً داخل rawCash أدناه ⟹ ازدواج
    // يكسر الهوية في كل السنوات (اختلال ثابت = رأس المال العامل، وشارة «غير متوازنة» حمراء
    // للمستخدم في BalanceSheetView). التمييز الآن بوجود الحقل لا بكونه غير صفري.
    const fixedAssetsGross = (Number.isFinite(capex.subtotal) ? capex.subtotal : (capex.total || 0)) + cumulativeReplacement;
    ...
    const accountsReceivableRaw = Math.max(0, Number(cashCycle?.receivables) || 0);
    ...
    const inventoryRaw = cashCycle
        ? Math.max(0, Number(cashCycle.inventory) || 0)
        : Math.max(0, openingInventory);
    const accountsPayableRaw = Math.max(0, Number(cashCycle?.payables) || 0);

    // استرداد رأس المال العامل عند نهاية الأفق (قرار مالك المنتج 2026-08-24، رقم 1؛ تنفيذ
    // 2026-09-16): قائمة التدفقات (nwcRecapture في engine.js) تفترض بالفعل تحصيل الذمم
    // المدينة وتصفية المخزون وسداد الذمم الدائنة بالكامل في آخر سنة من الأفق — لكن الميزانية
    // كانت تستمر بعرض الثلاثة كأن المشروع مستمر للأبد، فتنشأ فجوة تسوية بين القائمتين لنفس
    // السنة. تُصفَّر الثلاثة هنا في آخر سنة فقط (وفقط إن وُجد ما يُصفَّى أصلاً)؛ الفرق
    // (AR + المخزون − AP) يدخل النقدية تلقائياً عبر صيغة rawCash أدناه بلا أي تعديل عليها —
    // الهوية المحاسبية (الأصول = الخصوم + حقوق الملكية) تبقى صحيحة بنيوياً لأن AR/AP كانا
    // أصلاً يُلغيان أثرهما على إجمالي الأصول (انظر تعليق rawCash)، فتصفيرهما معاً لا يغيّر
    // ذلك الإلغاء، فقط يحوّل قيمتهما الصافية إلى نقدية صراحة.
    const isLiquidationYear = isLastYear &&
        (accountsReceivableRaw > 0 || inventoryRaw > 0 || accountsPayableRaw > 0);
    const nwcRecaptured = isLiquidationYear
        ? (accountsReceivableRaw + inventoryRaw - accountsPayableRaw)
        : 0;
    const accountsReceivable = isLiquidationYear ? 0 : accountsReceivableRaw;
    const inventory = isLiquidationYear ? 0 : inventoryRaw;
    const accountsPayable = isLiquidationYear ? 0 : accountsPayableRaw;
    ...
    if (loanSchedule?.annualSummary) {
        // ترحيل آخر رصيد معروف بدل افتراض 0 حين لا يوجد صف مطابق للسنة (year > termYears) —
        // يحدث فعلياً حين تغطي فترة السماح كامل مدة القرض فلا يُسدَّد أي جزء من الأصل خلالها
        // (مثال حقيقي: termYears=2, gracePeriodMonths=24)، فيسقط الالتزام محاسبياً من العدم
        // وتختل Assets = Liabilities + Equity بمقدار الرصيد كاملاً. نفس نمط
        // outstandingDebtAtHorizon المُثبَت في web/js/core/financial/cashflow.js (يُستخدم
        // هناك لتصحيح NPV/IRR من نفس الفجوة بالضبط).
        let endingBalance = 0;
        let bestYear = -Infinity;
        for (const row of loanSchedule.annualSummary) {
    ...
    return {
        ...
        // استرداد رأس المال العامل عند التصفية (0 في كل سنة غير أخيرة، أو حين لا يوجد
        // شيء ليُصفّى) — يُفصَح عنه صراحةً بدل أن يظهر كاختفاء غير مُفسَّر لـAR/المخزون.
        nwcRecaptured: Math.round(nwcRecaptured),
