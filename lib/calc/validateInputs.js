...
function isExplicitDiscountContext(parent, key, path) {
  ...
  // إعدادات السيناريوهات/الحساسية سالبة بطبيعتها المشروعة (تغيّرات نسبية مثل -20%)
  // وليست مبالغ مالية — بدون هذا الاستثناء كانت كل دراسة تُنذر زوراً عند التصدير.
  const p = String(path || "");
  if (/^scenarios\./.test(p) && /(revenueChange|costChange|range)/.test(p)) return true;
  ...
  while (stack.length) {
    ...
    if (typeof v === "object") {
      Object.keys(v).forEach((k) => {
        // تدقيق 2026-07-17: state.results هو مخرجات المحرك المخزَّنة (NPV/IRR/صافي الدخل/
        // التدفقات...) لا مدخلات مستخدم — قيم سالبة هنا (دراسة NO-GO حقيقية مثلاً) طبيعية
        // ومتوقعة تماماً. كان يُفحص بنفس قاعدة "لا قيم سالبة في المدخلات" فيُنتج عشرات
        // الأخطاء الحرجة الزائفة على أي دراسة غير مجدية عند التصدير فقط لأن نتيجتها سالبة.
        // تدقيق 2026-09-16: نفس العلة تماماً أصابت state.monteCarlo.lastRun — نتائج
        // محاكاة سالبة حقيقية (خسارة مرجّحة) كانت تُحجب القرار كـ"قيمة سالبة غير مسموحة"
        // كأنها مُدخَل خاطئ لا نتيجة حساب.
        if (path === "" && (k === "results" || k === "monteCarlo")) return;
        stack.push({ v: v[k], path: path ? `${path}.${k}` : k, parent: v, key: k });
      });
      continue;
    }
