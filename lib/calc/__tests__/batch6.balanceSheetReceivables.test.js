...
describe('balanceSheet — accountsReceivable من الدورة النقدية الفعلية (لا صفر ثابت)', () => {
    ...
    it('B2B بتحصيل آجل 60 يوماً ⇒ accountsReceivable لكل سنة = إيراد تلك السنة الفعلي × DSO/365 (ينمو مع growthRate 5%) حتى آخر سنة حيث تُصفَّى، والميزانية متوازنة', () => {
        const r = calculateStudy(makeStudy({
            assumptions: {
        ...
        r.balanceSheets.forEach((bs, idx) => {
            const year = idx + 1;
            if (year === lastYear) return; // آخر سنة: تُصفَّى عمداً (انظر التوكيد المخصَّص أدناه)
        ...
        // آخر سنة (تحديث سلوك منتج متعمَّد 2026-09-16 — قرار مالك المنتج "تعديل الميزانية"):
        // AR تُصفَّى بالكامل (تُحصَّل نقداً) لتعكس نفس افتراض التصفية الذي تفترضه قائمة
        // التدفقات (nwcRecapture في engine.js) أصلاً في آخر سنة من الأفق — هذا ليس عودة
        // للعلة الأصلية (AR=0 دائماً)؛ السنوات 1-4 أعلاه ما زالت تثبت رقماً حقيقياً متنامياً.
        const lastBs = r.balanceSheets[lastYear - 1];
        expect(lastBs.assets.current.accountsReceivable).toBe(0);
        expect(lastBs.nwcRecaptured).toBeGreaterThan(0);
        expect(lastBs.isBalanced, `السنة ${lastBs.year} غير متوازنة بفرق ${lastBs.imbalance}`).toBe(true);

        // تأكيد صريح للسلوك الجديد المتعمَّد: AR تنمو فعلياً عاماً بعد عام حتى قبيل آخر سنة
        // (لا تبقى مجمَّدة على رقم السنة الأولى كما كان الافتراض القديم في هذا الاختبار).
        for (let i = 1; i < accountsReceivableByYear.length - 1; i++) {
