...
describe('Restaurant Logic', () => {
  ...
  it('applies delivery commission ONLY to delivery channel', () => {
    ...
    // Packaging (1 per order): the engine charges packagingPerOrder against TOTAL
    // orders across every channel (totalOrders), not just the delivery channel --
    // there is no per-channel packaging carve-out for dine-in. Here dineIn(10/day)
    // + delivery(10/day) = 20 orders/day => 1 * 20 * 30 * 12 = 7,200
    // (NOT 1 * 10(del) * 30 * 12 = 3,600 as a dine-in-exempt reading would suggest).
    //
    // Also note: the engine's totalCOGS -- and therefore grossProfit = revenueTotal
    // - totalCOGS -- already nets out delivery commission (totalCOGS = foodCost +
    // wasteCost + packagingCost + commissionTotal), so "gross profit" here is NOT
    // "Rev - Food - Waste - Packaging" alone; it also deducts the 72,000 commission.
    //
    // Total COGS = 216,000 (food) + 4,320 (waste) + 7,200 (packaging) + 72,000 (commission) = 299,520
    // GP = 720,000 - 299,520 = 420,480

    const y1Gross = res.pnl[0].grossProfit;
    expect(y1Gross).toBeCloseTo(420480, -2);
  ...
  it('aggregates food cost, waste, and packaging correctly', () => {
    ...
    const res = computeRestaurantBase(model);
    // totalCOGS = food + waste + packaging (flat field on the pnl entry, not a nested `cogs` object)
    const y1COGS = res.pnl[0].totalCOGS;
