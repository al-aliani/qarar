...
describe('ΔNWC — دراسة بلا سياسة دورة نقدية: لا ΔNWC سنوي، لكن الاسترداد التأسيسي مستحق', () => {
    // تحديث 2026-08-25 (فكّ اقتران الاسترداد بوجود سياسة نقدية): الصيغة الأصلية لهذا
    // الاختبار كانت تثبّت nwcRecapture === 0 لكل سنة حين hasCashCycle=false — وهو بالضبط
    // التفضيل البنيوي الذي أُصلح: مشروع بلا سياسة نقدية كان لا يسترد رأس ماله العامل
    // إطلاقاً بينما مشروع بيوم DSO واحد يستردّه، فيقلب إشارة NPV بمدخل واحد. الآن الاسترداد
    // مستحق في السنة الأخيرة بصرف النظر عن مسار الإدخال. ما يبقى ثابتاً بحق (وهو ما يحمي
    // الدراسات بلا سياسة من أي تغيير غير مقصود): deltaNWC صفر لكل سنة، والاسترداد صفر في
    // كل سنة ما عدا الأخيرة، وقيمته هناك = رأس المال العامل التأسيسي بالضبط (لا أكثر).
    it('hasCashCycle=false ⇒ cashCycle=null، deltaNWC صفر لكل سنة، nwcRecapture صفر إلا في السنة الأخيرة حيث = capex.workingCapital', () => {
        ...
        const lastYear = r.incomeStatement.length;
        expect(r.capex.workingCapital).toBeGreaterThan(0);

        r.incomeStatement.forEach(stmt => {
            expect(stmt.deltaNWC).toBe(0);
            const expectedRecapture = stmt.year === lastYear ? r.capex.workingCapital : 0;
            expect(stmt.nwcRecapture).toBeCloseTo(expectedRecapture, 6);
            // الصيغة: netCashFlow = operatingCF + financingCF - replacementCost + nwcRecapture
            // = (netIncome + depreciation) + (-loanPrincipalPaid) - replacementCost + nwcRecapture
            const expectedCashFlow = stmt.netIncome + stmt.depreciation - stmt.loanPrincipalPaid
                - stmt.replacementCost + expectedRecapture;
...
describe('ΔNWC — الميزانية تبقى متوازنة، وaccountsPayable لم يعد صفراً', () => {
    it('isBalanced=true و|imbalance|<=5 لكل سنوات الأفق، وaccountsPayable > 0 حين dpoDays > 0 (باستثناء آخر سنة: تُصفَّى عمداً)', () => {
        const r = calculateStudy(makeB2BStudyWithLoanAndCashCyclePolicy());

        expect(r.balanceSheets.length).toBe(5);
        const lastYear = r.balanceSheets.length;
        r.balanceSheets.forEach(bs => {
            ...
            if (bs.year === lastYear) {
                // آخر سنة: تُصفَّى عمداً (تحديث سلوك منتج متعمَّد 2026-09-16 — قرار مالك المنتج
                // "تعديل الميزانية") فتُسدَّد الذمم الدائنة بالكامل، بنفس افتراض التصفية الذي
                // يفترضه nwcRecapture أصلاً في قائمة التدفقات — لا عودة للعلة الأصلية (AP=0
                // دائماً بصرف النظر عن dpoDays)، فالسنوات السابقة أدناه ما زالت تثبت العكس.
                expect(bs.liabilities.current.accountsPayable).toBe(0);
                expect(bs.nwcRecaptured).toBeGreaterThan(0);
            } else {
                // ذمم الموردين لم تعد مقفلة على صفر (العلة الأصلية) — موجبة فعلياً لكل سنة تشغيلية
