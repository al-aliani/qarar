...
// تدقيق 2026-07-08 (ملاحظة متوسطة #8): loanExists/computeDscrMin كانتا تقرآن
// مسارات قديمة (inputs.financing.loan، outputs.dscr، outputs.kpis.dscrMin) لا
// تطابق مخطط القرض الفعلي (financing.sources.bankLoan) ولا شكل مخرجات المحرك
// الحقيقي (indicators.dscr سنة أولى، dscrAnalysis مصفوفة سنوية) — فلا يعمل هذا
// الفحص الحرج إطلاقاً على أي دراسة حقيقية.
function loanExists(inputs) {
  const loan = inputs?.financing?.sources?.bankLoan || inputs?.financing?.loan || inputs?.loan || null;
  const amount = toNum(loan?.amount);
  return Number.isFinite(amount) && amount > 0;
}

function computeDscrMin(outputs) {
  const analysis = Array.isArray(outputs?.dscrAnalysis) ? outputs.dscrAnalysis : [];
  if (analysis.length) {
    const vals = analysis.map((x) => toNum(x?.dscr)).filter((x) => Number.isFinite(x) && x > 0);
    if (vals.length) return Math.min(...vals);
  }
  const v = toNum(outputs?.indicators?.dscr);
