-- ============================================================
-- ⚠️ ميزة مستقبلية غير مفعّلة — لا يستخدمها الكود بعد، وتحتاج توفيقاً قبل التشغيل.
-- ============================================================
-- ينشئ study_folders + study_shares، لكن سياساته تفترض studies.owner_id بينما
-- المخطط الكنسي (docs/supabase_setup.sql) يستخدم studies.user_id. قبل تشغيله
-- بدّل owner_id → user_id في مراجع studies. لاحظ أن supabase_setup.sql يعرّف
-- study_shares أصلاً، فلا تُنشئه مرتين.
--
-- Runway-style: مشاركة الدراسة + مجلدات (اختياري — نفّذ بعد التوفيق)
-- ============================================================
