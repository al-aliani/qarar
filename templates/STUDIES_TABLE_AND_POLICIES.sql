-- ═══════════════════════════════════════════════════════════════════════
-- ⚠️ مهجور ومتعارض مع المخطط الحي — لا تُشغِّل هذا الملف. استخدم
-- supabase/migrations/ بدلاً منه (المصدر الوحيد المُتتبَّع والمُطبَّق فعلياً،
-- انظر تحديداً 20260708080000_studies_table_bootstrap.sql +
-- 20260708090000_enable_rls_studies.sql لجدول public.studies الحقيقي، أو
-- docs/supabase_setup.sql للمخطط الكنسي الموثَّق الكامل).
--
-- دفعة 7 (2026-08-27، نظافة قاعدة البيانات): هذا الملف يفترض عمود owner_id
-- وقيماً مختلفة لعمود status ('final' بدل 'completed') وعمود template_slug
-- غير موجود إطلاقاً في الجدول الحي — الجدول الحقيقي يستخدم user_id (انظر
-- web/js/services/PersistenceService.js._saveCloud). تشغيل هذا الملف ضد
-- قاعدة الإنتاج الحية سيُنفِّذ فعلياً ALTER TABLE...ADD COLUMN IF NOT EXISTS
-- (يُضيف template_id/template_version للجدول الحي بصمت) قبل أن يفشل لاحقاً
-- بخطأ "column owner_id does not exist" عند محاولة إنشاء السياسات — يُعدِّل
-- المخطط الحي جزئياً ثم يتوقف بمنتصف الطريق، لا يفشل بأمان من البداية.
--
-- أُبقي هذا الملف لسياقه التاريخي فقط (كان تصميماً مبكراً بديلاً لم يُطبَّق) —
-- لا يُحذَف بلا تأكيد صريح، لكن لا يُشغَّل بعد الآن.
-- ═══════════════════════════════════════════════════════════════════════
